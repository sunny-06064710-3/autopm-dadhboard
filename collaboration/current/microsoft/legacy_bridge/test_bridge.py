...
class BridgeTests(unittest.TestCase):
    ...
    def test_task_issue_and_project_mapping(self):
        ...
        self.assertEqual(len(tasks),3);self.assertEqual(len(issues),1)
        p1=next(x['new'] for x in tasks if x['display_new']=='P1 BUILD DATE');self.assertEqual(p1[b.TASK_FIELDS['due']],'2026-09-17')
        mp=next(x['new'] for x in tasks if x['display_new']=='MP Start');self.assertEqual(mp[b.TASK_FIELDS['due']],'2026-10-19');self.assertEqual(mp[b.TASK_FIELDS['completed_by']],['per1'])
