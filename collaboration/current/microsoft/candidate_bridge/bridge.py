...
def make_airtable_plan(parsed,snapshot):
    ...
    changes=[]; skipped=[]; blockers=[]; reported_completion=[]
    ...
    for p in parsed['projects']:
        ...
        for entry in p.get('dated_tasks',[]):
            ...
            # BR-07: a point milestone is one day; a task needs a defined range.
            # A source date never implies an arbitrary seven-day duration.
            start_date=entry.get('start_date');due_date=entry.get('due_date')
            if not (start_date and due_date):
                if milestone or entry.get('date_role')=='single_day':
                    start_date=due_date=entry.get('date')
                else:
                    skipped.append({'pid':key,'reason':'task_date_range_requires_mapping','task_name':task_name,'source':entry})
                    continue
            if not start_date or not due_date or dt.date.fromisoformat(due_date)<dt.date.fromisoformat(start_date):
                skipped.append({'pid':key,'reason':'invalid_task_date_range','task_name':task_name,'source':entry})
                continue
            if entry.get('green'):
                reported_completion.append({'pid':key,'task_name':task_name,'source_cell':entry.get('cell'),'claim':'source_reported_complete','requires_confirmation':True})
    ...
    return {'kind':'weekly_to_airtable','rule_version':'BR-2026-09-11','reported_completion':reported_completion,'base':snapshot['base'],'table':snapshot['table'],'tasks_table':snapshot.get('tasks_table'),'issues_table':snapshot.get('issues_table'),'source':parsed['source'],'source_sha256':parsed['sha256'],'changes':changes,'skipped':skipped,'warnings':parsed['warnings'],'blockers':unique_blockers}
