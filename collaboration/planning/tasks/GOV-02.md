...
- PM/Owner/验证人/只读/管理员
- Interface/Base与Dataverse/文件权限
- 测试/生产边界
- 支持与发布

## 4. 具体步骤

1. 定义角色对实体/字段/业务操作的读写矩阵与独立核验责任
2. 核对申请状态/运行许可/服务身份，未批准逐项记录
3. 规定测试发送/外部写回隔离及生产发布证据要求
4. 安排真实责任承接和恢复联系路径，未安排保留角色占位

## 5. 范围边界

不索取广泛租户管理员权限、不自动发邮件或替人接受职责；当前无真实人员分派。
...
- environment-gap.md
- release-policy.md
- support-responsibility.md

统一提交位置：`submissions/GOV-02/<提交版本>/`，使用[DELIVERY模板](../templates/DELIVERY.md)。源码保存在获准开发分支，此处给版本和路径；失败/未运行分开，不附凭据。

## 7. 验收要求

用例：[TC-23](../acceptance/TEST_CATALOG.md#tc-23), [TC-25](../acceptance/TEST_CATALOG.md#tc-25), [TC-31](../acceptance/TEST_CATALOG.md#tc-31)。逐例按用例库输入和期望验证，提交Actual及证据层级。离线通过不代表平台通过；平台组件通过不代表全链路／两周期通过。依赖或环境未满足时可审核子成果，完整包不标Accepted。

恢复要求：保存变更前配置、受影响记录与版本，定义补偿/恢复步骤并保护后续合法修改；纯规划包说明版本撤回和受影响后继任务。

## 8. 需求、历史任务与候选资产

需求：REQ-026 / REQ-027 / REQ-028 / REQ-029。历史Action映射：M1.1-A05, M1.5-A03, M4.6-A01, M4.6-A02, M4.6-A03, M4.7-A02, M4.7-A03, M4.7-A05, M4.7-A06, M5.2-A01, M5.3-A01。
